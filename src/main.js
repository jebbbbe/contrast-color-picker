...
function render() {
    ...
    materials[0].customUniforms.u_camPos.value.copy(camera.position);

    // Update camera direction using getWorldDirection().
    camera.getWorldDirection(materials[0].customUniforms.u_camDir.value);

    // Convert the camera's field of view from degrees to radians.
    materials[0].customUniforms.u_fov.value = camera.fov * Math.PI / 180.0;
...
function addMesh() {
    ...
    const material = materials[0] = new sdfRenderMaterial({
        // u_camPos: { value: M.virtualPosition },
        // u_camDir: { value: M.virtualDir },
        // u_fov: { value: M.virtualFov },
    })
